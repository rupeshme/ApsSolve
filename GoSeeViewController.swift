//
//  GoSeeViewController.swift
//  ApsSolve
//
//  Created by Rupesh on 2/5/19.
//  Copyright © 2019 Rupesh. All rights reserved.
//

import UIKit
import QuickLook
class GoSeeViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UITextFieldDelegate,UITextViewDelegate,QLPreviewControllerDataSource {

    @IBOutlet weak var  tableView : UITableView!
    var gosee  = currentProblem["gosee"] as! NSMutableDictionary
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(indexPath.row==0)
        {
            ...
            textField.text = gosee["whodetected"] as? String
    ...
    @IBAction func evalute(_ sender : UIButton)
    {
        let evaluteViewController = self.storyboard?.instantiateViewController(withIdentifier: "evaluteViewController")
        self.navigationController?.pushViewController(evaluteViewController!, animated: true)
    }
    ...
    func textFieldDidEndEditing(_ textField: UITextField)
    {
        ...
        {
            let textField = cell!.contentView.viewWithTag(1) as! UITextField
            gosee["whodetected"] = textField.text!
    ...
    func textViewDidBeginEditing(_ textView: UITextView) {
        ...
        {
            if textView.text == "-process step/operation\n-device(sense or instrument)" {
                textView.text = ""
                textView.textColor = UIColor.black
            }
        }
    ...
    func textViewDidEndEditing(_ textView: UITextView)
    {
         ...
         if(indexPath?.row == 1)
        {
            ...
            gosee["wheredetected"] = textView.text!
            //+ "<br>" + textField2.text!
            if textView.text == "" {
                textView.text = "-In the area\n-On the part"
                textView.textColor = UIColor.lightGray
            }
        }
         else if(indexPath?.row == 4)
         {
            gosee["howdetected"] = textView.text!
            if textView.text == "" {
                textView.text = "-process step/operation\n-device(sense or instrument)"
                textView.textColor = UIColor.lightGray
            }
        }
